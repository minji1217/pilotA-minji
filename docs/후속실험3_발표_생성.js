...
const NAMES = ["1 3번 b4","2 3번 b10","3 6번 b10","4 3번 자연혼재","5 6번 자연혼재",
  ...
  "G LQ c=0.5","H LQ c=0.75","I LQ c 학습","J b만 학습","K LQ c=0.62"];
const ZP_LS   = [0.7969,0.7969,0.7969,0.8883,0.8883,0.8400,0.8400,0.8163,0.7969,0.8163,0.8679,0.8400,0.8400,0.8400,0.8400,0.8400];
const POST_LS = [0.7348,0.7635,0.7506,0.8474,0.8481,0.8550,0.8204,0.8031,0.7710,0.7915,0.8658,0.8550,0.8507,0.8550,0.8235,0.8550];
const ZP_LQ   = [0.7154,0.7154,0.8372,0.7154,0.8372,0.7698,0.7698,0.7709,0.7784,0.7570,0.7687,0.7570,0.7725,0.7547,0.7698,0.7702];
const POST_LQ = [0.7504,0.7663,0.7753,0.7663,0.7753,0.7812,0.7871,0.7813,0.7814,0.7914,0.7681,0.7905,0.7955,0.7732,0.8100,0.7902];
const BIAS_LS = [-0.308,-0.210,-0.211,0.151,0.148,0.006,-0.270,-0.269,-0.192,-0.494,0.146,0.008,0.007,0.008,-0.273,0.007];
const BIAS_LQ = [-0.180,-0.204,-0.204,-0.204,-0.204,0.256,-0.229,-0.214,-0.208,-0.103,0.501,-0.106,0.104,-0.166,-0.192,0.002];
...
/* ══════════ 2. 한 장 요약 ══════════ */
{
  ...
  const items = [
    ["1", "유도식 c = 1 이 산사태에서 정확히 맞는다",
     "c를 바꿔 본 세 조건이 c = 1을 가리킨다. E(c=0.5) 편향 −0.494 · A(c=1) +0.006 · F(c=2) +0.146. A는 MSE도 0.0998로 기저확률 0.1344를 크게 밑돈다. “격자 칸끼리 독립” 가정이 산사태에서는 성립한다.", LS],
    ["2", "액상화는 c 를 0.62로 낮춰야 한다",
     "A(c_LQ=1.0) 편향 +0.256 → G(0.5) −0.106 → H(0.75) +0.104. 부호가 반대니 사이에 0이 있고, 선형보간하면 0.626이다. 실제로 0.62를 돌린 것이 K로 편향 +0.002 — 두 hazard 모두 0.007 안에 드는 유일한 조건이다.", LQ],
    ["3", "b 를 학습하면 오히려 나빠진다",
...
{
  const s = slideBase(false);
  head(s, "실험", "세 갈래 16개 조건", false);
  const groups = [
    ["②","교수님 피드백 — b 범위","3개","b 상한을 4 / 10으로 넓혀 안쪽에서 멈추는지 본다","교수님 지시",LS],
    ["③","자연+혼재 GT 재평가","2개","②의 모델을 그대로 두고 LS 정답만 자연사면으로 좁힌다","교수님 지시",GOOD],
    ["④","면적 항","11개","z = a·log p̄ + b + c·log k 의 a·b·c를 어떻게 다룰지","A~F 지시 / G~K 추가",LQ],
  ];
  groups.forEach(([n,t,cnt,d,src,c],i)=>{
    const y = 2.25 + i*1.45;
    card(s, { x: M, y, w: 12.1, h: 1.25, fill: BG2 });
    s.addShape(pres.ShapeType.ellipse, { x: M+0.3, y: y+0.4, w: 0.46, h: 0.46, fill: { color: c } });
    s.addText(n, { x: M+0.3, y: y+0.46, w: 0.46, h: 0.34, isTextBox: true, margin: 0,
      align: "center", fontFace: KR, fontSize: 14, bold: true, color: "FFFFFF" });
    s.addText(t, { x: M+0.95, y: y+0.28, w: 5.0, h: 0.34, isTextBox: true, margin: 0,
      fontFace: KR, fontSize: 15, bold: true, color: INK });
    s.addText(cnt + " · " + src, { x: M+0.95, y: y+0.68, w: 5.0, h: 0.3, isTextBox: true, margin: 0,
      fontFace: KR, fontSize: 11, color: INK3 });
    s.addText(d, { x: M+6.2, y: y+0.42, w: 5.6, h: 0.6, isTextBox: true, margin: 0,
...
/* ══════════ 9. 갈래 ④ 상세 ══════════ */
{
  ...
  const rows = [
    ...
    ["확정","K","lq-c062","avg-lq-c062","1.00","0.00","1.00","1.00","0.00","0.62","0","G·H가 친 괄호 사이를 찍어 눈금을 맞춘다"],
  ];
  const fill = { "지시":"FAFCFD", "추가":"FFF6EC", "요청":"EAF4EE", "확정":"E8F1F6" };
  s.addTable([
  ...
  s.addText("출처 —  회색: 교수님이 지시하신 A~F     주황: 실험 도중 만든 G·H·I     초록: 요청으로 추가한 J     파랑: G·H 결과로 확정한 K", {
...
{
  const s = slideBase(false);
  head(s, "갈래 ④", "G · H · I · J 는 왜 그 값으로 정했나 — 그리고 K 로 끝난다", false);
  ...
  s.addText("G(0.5) −0.106 과 H(0.75) +0.104 는 부호가 반대다 — 사이에 0 이 있다.  선형보간하면 0.5 + 0.25×0.106/(0.106+0.104) = 0.626,  실제로 0.62 를 돌린 것이 K 로 편향 +0.002 다.  I 가 학습한 0.434 는 −0.166 으로 아래로 지나쳤다 — 우도에 맡기면 눈금이 맞는 자리로 가지 않는다.", {
...
{
  const s = slideBase(false);
  head(s, "갈래 ④", "대부분의 조건이 왜 b 를 0 으로 못박았나", false);
  ...
  const why = [
    ...
    ["②", "b 를 학습시키면 눈금이 오히려 망가진다",
     ...
     "b = 0 인 A · G · H · I 는 +0.006 ~ +0.008 이다." + NL +
    ...
    ["③", "b 가 자유로우면 c 를 잴 수 없다",
     ...
     "즉 c 도 거의 b 처럼 행동한다. 둘 다 자유면 “편향 0”을 만드는 (b, c) 짝이 무수히 많아 어느 쪽이 맞는지 가릴 수 없다." + NL +
     "b 를 0 에 박아야 편향이 c 를 가리키는 자가 된다 — E · F · G · H · I 가 그래서 전부 b = 0 이다.", LQ],
  ];
  why.forEach(([n, t, d, col], i) => {
    const y = 2.50 + i * 1.56;
    note(s, M, y, 12.1, 1.50, n + "  " + t, d, col);
  });
  s.addText("J 가 바로 “b 가 올바른 손잡이인가”를 시험한 조건이었고, 편향 −0.273 으로 답은 아니오였다.  눈금을 고치려면 b 가 아니라 c 를 움직여야 한다.", {
...
/* ══════════ 15. 전체 결과 표 ══════════ */
{
  ...
  const rows = [
    ...
    ["K","LQ c=0.62","125","0.797","0.840","0.855","+0.015","0.0996","+0.007","0.790","+0.020","0.1764","+0.002"],
  ];
  const hi = new Set(["A","G","H","I","K"]);
  ...
  s.addTable([
    ...
    ...rows.map(r=>r.map((c,i)=>({text:c,
      options:{align:i>=2?"right":"left",fontFace:i>=2?NUM:KR,
       ...
       rowH:0.275, fontFace:KR, fontSize:9.5, valign:"middle", border:{type:"solid",color:"E3E9EC",pt:1} });
...
/* ══════════ 15-2. 정답 규칙이 순위를 뒤집는다 ══════════ */
{
  ...
  const rows = [
    ...
    ["E","c=0.5","0.7915","0.4192","−0.494","0.8419","0.1069","−0.030","면적 영향이 작아 확률이 낮다"],
    ["B","tied","0.8204","0.2148","−0.270","0.8389","0.1382","+0.156","b 학습"],
    ["J","b만 학습","0.8235","0.2218","−0.273","0.8391","0.1429","+0.150","b 학습"],
    ["D","free","0.7710","0.2527","−0.192","0.8599","0.1473","+0.158","b 학습"],
    ["A","fixed","0.8550","0.0998","+0.006","0.8541","0.4162","+0.503","b = 0 고정 — 높이를 못 바꾼다"],
    ...
    ["K","LQ c=0.62","0.8550","0.0996","+0.007","0.8559","0.4166","+0.503","b = 0 고정"],
    ["F","c=2","0.8658","0.1443","+0.146","0.8130","0.7738","+0.782","면적을 두 배로 — 양쪽 다 과대"],
  ];
  s.addTable([
    ["","조건","AUC","MSE","편향","AUC","MSE","편향","왜"].map((h,i)=>
      ({text:h,options:{bold:true,color:INK,
        fill:{color:i>=2&&i<=4?"F6F1EF":i>=5&&i<=7?"EDF4F7":BG2},
